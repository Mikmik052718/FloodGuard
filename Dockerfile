...
# 4. Enable Apache Rewrite (needed for C14 pretty URLs)
RUN a2enmod rewrite
...
# 7. Install PHP dependencies according to composer.lock (force
reinstall)
...
-no-cache \
&& Is -la /app/vendor \
&& Is -la /app/vendor/react || true \
&& Is -la /app/vendor/react/promise || true
