...
<table id="predictionTable" style="border-collapse:collapse;margin:auto;">
  <thead>
    <tr>
      <th>Date</th>
      <th>Prob&nbsp;%</th>
        ...
        fetch("<?= site_url('flood/predict-ajax') ?>")
          ...
          .then(data => {
              ...
              if (data.status === 'error') {
                  tbody.innerHTML = `<tr><td colspan="8" style="color:red;text-align:center;">Error: ${data.message}</td></tr>`;
                  return;
              }
              data.days.forEach(d => {
                  const row = document.createElement("tr");
                  row.className = (d.prediction === "FLOOD") ? "flood" : "no-flood";
                  row.innerHTML = `
                    <td>${d.date}</td>
          ...
          .catch(err => {
              document.querySelector("#predictionTable tbody").innerHTML = `<tr><td colspan="8" style="color:red;text-align:center;">Failed to load prediction data.</td></tr>`;
              console.error(err);
          });
    });
